...
void WinResult3D::Initialize()
{
	Master::mpCamera->Reset();
	mnHandle_V = Graph::VictoryPhoto(Master::msPlayerModelName);
...
void WinResult3D::Draw()
{
	...
	{
		DrawString(Utility::SCREEN_WIDTH / 2 + 100, Utility::SCREEN_HEIGHT / 2 + 300, "Enterでタイトル", Cr2);
	}

	if (mbRoadFlag)
	{
		DrawString(Utility::SCREEN_WIDTH / 2, Utility::SCREEN_HEIGHT / 2, "ロード中です", Cr2);
	}  
	...
	int totalNumber = (resultEnemyCount + resultPlayerHp + resultTime) + 4000; // 勝利ボーナス

	// スコアの代入
	m_Score.ScoreGet(resultTime, resultPlayerHp, resultEnemyCount);

	// VICTORY
	DrawFormatStringToHandle((int)X,200,Cr2,mnFontBig,"VICTORY");
	...
	for (const auto& i : slides)
	{
		...
		{
			float x = m_Slide.SlideDirection(i.SlideStart, i.SlideEnd, startX, endX);
			DrawFormatStringToHandle((int)x, i.SlideY, Cr2, i.SlideFont, "%s", i.moji.c_str());
		}
	}
