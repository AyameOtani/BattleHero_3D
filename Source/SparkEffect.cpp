...
SparkEffect::SparkEffect(VECTOR initPos, std::string filename)
	: Object3D(initPos)
{
	...
	for (int i = 0; i < PARTICLE_NUM; i++) // 回数を回してほしい
	{
		...
		// 進行方向
		mpEffect->particle[i].dir.x = ((float)GetRand(200) - 100.0f) / 250.0f;
		mpEffect->particle[i].dir.y = ((float)GetRand(200) - 100.0f) / 250.0f;
		mpEffect->particle[i].dir.z = ((float)GetRand(200) - 100.0f) / 250.0f;

		// 速度
		mpEffect->particle[i].speed = ((float)GetRand(SPEED_RAND_MAX) + SPEED_RAND_MIN) / 100.0f;

		// 不透明度 
		mpEffect->particle[i].alpha = 0.0f;

		// 大きさ
		mpEffect->particle[i].size = ((float)GetRand(SIZE_RAND_MAX) + SIZE_RAND_MIN) / 50.0f;

		// 表示時間
		mpEffect->particle[i].visibleTime = ((float)GetRand(VISIBLE_TIME_RAND_MAX) + VISIBLE_TIME_RAND_MIN) / 100.0f;
	}
...
void SparkEffect::Update()
{
	...
	for (int i = 0; i < PARTICLE_NUM; i++)
	{
		...
		if (mpEffect->particle[i].alpha <= .0f)
		{
			continue;
		}
		...
		{
			// 表示時間を減らしていく
			mpEffect->particle[i].visibleTime -= 0.98 * stepTime;
			mpEffect->particle[i].size *= 0.98;
		}
		else
		{
			// アルファ値を落としていく(徐々に透明にしていく)
			mpEffect->particle[i].alpha -= 8.0f * stepTime;
		}
...
void SparkEffect::Draw()
{
	...
	for (int i = 0; i < PARTICLE_NUM; i++)
	{
		...
		// 輝度は最大４倍になる
		// ビルボード　　カメラの方向に常に向いてくれる３Dポリゴン ★重要
		SetDrawBlendMode(DX_BLENDMODE_ADD_X4, (int)mpEffect->particle[i].alpha * 255.0f);
