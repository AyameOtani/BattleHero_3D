...
Effect4::Effect4(VECTOR initPos, std::string filename)
	: Object3D(initPos)
{
	mpEffect = new EffectInfo();

	mpEffect->color = GetColorU8(50, 100, 100, 100); // 今はでオレンジ色っぽい色
	...
	for (int i = 0; i < PARTICLE_NUM; i++) // 回数を回してほしい
	{
		...
		// 大きさ
		mpEffect->particle[i].size = ((float)GetRand(SIZE_RAND_MAX) + SIZE_RAND_MIN) / 40.0f;

		// 表示時間
		mpEffect->particle[i].visibleTime = ((float)GetRand(VISIBLE_TIME_RAND_MAX) + VISIBLE_TIME_RAND_MIN) / 500.0f;

		mpEffect->particle[i].angle = GetRand(360) * DX_PI_F / 180.0f; // 初期角度
...
void Effect4::Draw()
{
	...
	for (int i = 0; i < PARTICLE_NUM; i++)
	{
		...
		// 輝度は最大４倍になる
		// ビルボード　　カメラの方向に常に向いてくれる３Dポリゴン ★重要
		SetDrawBlendMode(DX_BLENDMODE_ADD_X4, (int)mpEffect->particle[i].alpha * 235);
