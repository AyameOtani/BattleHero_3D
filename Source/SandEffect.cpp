...
SandEffect::SandEffect(VECTOR initPos, std::string filename)
	: Object3D(initPos)
{
	mpEffect = new EffectInfo();

	mpEffect->color = GetColorU8(131, 91, 37, 255); // 今は固定で赤色っぽい色
	...
	for (int i = 0; i < PARTICLE_NUM; i++) // 回数を回してほしい
	{
		...
		// 大きさ
		mpEffect->particle[i].size = ((float)GetRand(SIZE_RAND_MAX) + SIZE_RAND_MIN) / 8.0f;
...
void SandEffect::Update()
{
	...
	for (int i = 0; i < PARTICLE_NUM; i++)
	{
		...
		{
			// 表示時間を減らしていく
			mpEffect->particle[i].visibleTime -= 0.65f * stepTime;
		}
		else
		{
		 ...
		 // 初めだけ上に待ってるようにしたい
		 if (mpEffect->particle[i].lifeTime < 0.2f)
		 {
			 mpEffect->particle[i].dir.y += 0.05f; // 上に行かせてる
		 }
		 else
		 {
			 mpEffect->particle[i].dir.y -= 0.3f; // 下に行かせてる
		 }
	}
...
void SandEffect::Draw()
{
	...
	for (int i = 0; i < PARTICLE_NUM; i++)
	{
		...
		// SetDrawBlendMode(DX_BLENDMODE_INVSRC, (int)mpEffect->particle[i].alpha * 2.0f);
		DrawBillboard3D(
