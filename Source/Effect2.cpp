...
Effect2::Effect2(VECTOR initPos, std::string filename)
	: Object3D(initPos)
{
	mpEffect = new EffectInfo();

	mpEffect->color = GetColorU8(50, 100, 200, 100); // 今はでオレンジ色っぽい色
	...
	for (int i = 0; i < PARTICLE_NUM; i++) // 回数を回してほしい
	{
		...
		// 進行方向
		mpEffect->particle[i].dir.x = ((float)GetRand(200) - 100.0f) / 80.0f;
		mpEffect->particle[i].dir.y = ((float)GetRand(200) - 100.0f) / 80.0f;
		mpEffect->particle[i].dir.z = ((float)GetRand(200) - 100.0f) / 80.0f;

		// 速度
		mpEffect->particle[i].speed = ((float)GetRand(SPEED_RAND_MAX) + SPEED_RAND_MIN) / 120.0f;

		// 不透明度 
		mpEffect->particle[i].alpha = 1.0f;

		// 大きさ
		mpEffect->particle[i].size = ((float)GetRand(SIZE_RAND_MAX) + SIZE_RAND_MIN) / 75.0f;

		// 表示時間
		mpEffect->particle[i].visibleTime = ((float)GetRand(VISIBLE_TIME_RAND_MAX) + VISIBLE_TIME_RAND_MIN) / 300.0f;
	
		// 色
		mpEffect->particle[i].r = GetRand(50);	// 0から50にした
		mpEffect->particle[i].g = GetRand(155) + 100; // 100から255
		mpEffect->particle[i].b = GetRand(20) + 40; // 20から60
...
void Effect2::Update()
{
	...
	for (int i = 0; i < PARTICLE_NUM; i++)
	{
		...
		{
			// 表示時間を減らしていく
			mpEffect->particle[i].visibleTime -= 0.76f * stepTime;
		}
		else
		{
			// アルファ値を落としていく(徐々に透明にしていく)
			mpEffect->particle[i].alpha -= 3.0f * stepTime;
		}

		mpEffect->particle[i].size -= 0.62f; // 小さく
...
void Effect2::Draw()
{
	...
	for (int i = 0; i < PARTICLE_NUM; i++)
	{
		...
		SetDrawBlendMode(DX_BLENDMODE_INVSRC, (int)(mpEffect->particle[i].alpha * 70.0f));
		DrawBillboard3D(
	...
	for (int i = 0; i < PARTICLE_NUM; i++)
	{
		...
		// 輝度は最大４倍になる
		// ビルボード　　カメラの方向に常に向いてくれる３Dポリゴン ★重要
		SetDrawBlendMode(DX_BLENDMODE_ADD_X4, (int)(mpEffect->particle[i].alpha * 150.0f));
